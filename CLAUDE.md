...
```bash
rtk turbo build         # Turborepo build output
rtk turbo run build      # Explicit turbo run
rtk tsc                 # TypeScript errors grouped by file/code (83%)
rtk lint                # ESLint/Biome violations grouped (84%)
rtk prettier --check    # Files needing format only (70%)
rtk turbo run build     # Full build with metrics (87%)
```

### Test (90-99% savings)

```bash
rtk turbo run test      # Turborepo test output
...
### Package Manager - pnpm (70-90% savings)
...
rtk pnpm scan:deps      # Compact dependency scanner output
rtk pnpm scan:deps:summary # Compact dependency summary
rtk pnpm format         # Compact prettier formatting output
rtk pnpm format:check   # Compact format check output
```

### Turborepo (85% savings)

```bash
rtk turbo run build     # Compact build output
rtk turbo run dev       # Compact dev output
rtk turbo run lint      # Compact lint output
rtk turbo run test      # Compact test output
rtk turbo run typecheck # Compact typecheck output
rtk turbo graph         # Compact dependency graph
```
...
This project uses Turborepo with pnpm. Key commands:

| Command                 | Description             |
| ----------------------- | ----------------------- |
| `pnpm build`            | Build all packages      |
| `pnpm dev`              | Run dev on all packages |
| `pnpm dev:mobile`       | Mobile app dev          |
...
| `pnpm scan:deps`        | Dependency scanner      |
...
| ---------------- | ---------------------------------- | --------------- |
| Tests            | vitest, playwright, turbo run test | 90-99%          |
| Build            | turbo, tsc, lint, prettier         | 70-87%          |
| Git              | status, log, diff, add, commit     | 59-80%          |
| GitHub           | gh pr, gh run, gh issue            | 26-87%          |
| Package Managers | pnpm, turbo run                    | 70-90%          |
