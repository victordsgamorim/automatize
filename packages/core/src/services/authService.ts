 ...
 */
export function validateTOTPCode(code: string): {
  valid: boolean;
  message?: string;
} {
 ...
 */
export function validateBackupCode(code: string): {
  valid: boolean;
  message?: string;
} {
  ...
  if (!/^[A-Z0-9]{8}$/.test(trimmed)) {
    return {
      valid: false,
      message: 'Backup code must contain only uppercase letters and numbers',
    };
  }
